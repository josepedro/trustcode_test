...
def products_values():
    ...
    print("Os seguintes produtos foram comprados pelo cliente de maior venda:")
    [client_major] = odoo.execute_kw('res.partner', 'read', [client_id_major_sale])
    sale_order_ids = client_major['sale_order_ids']
    for sale_order_id in sale_order_ids:
        [sale_order] = odoo.execute_kw('sale.order', 'read', [sale_order_id])
        name_product = sale_order['product_id'] 
